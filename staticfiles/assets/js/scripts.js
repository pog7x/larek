			...
			return {
				init: function () {
					...
					$select.on('change', function (e) {
						console.log($(e.target).attr('cart_id'), e.target.value);
						$.ajax({
							type: 'GET',
							url: `/cart/${$(e.target).attr('cart_id')}/products_count/?product_seller=${e.target.value}`,
			...
			return {
				init: function () {
					$next.add($navigate.find('.menu-link')).on('click', function (e) {
						...
						if (
							error === false &&
							($(e.target).is('.Order-next') || $blocks.index($(href)) < $blocks.index($blocks.filter('.Order-block_OPEN')))
						) {
